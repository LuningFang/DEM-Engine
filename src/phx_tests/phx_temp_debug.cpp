...
int main(int argc, char* argv[]) {
    ...
    std::pair<float, float> plate_y_range = std::make_pair(-24., -23.);
    ...
    for (double t = 0; t < (double)time_end; t += step_size, curr_step++) {
        ...
        if (curr_step % out_steps == 0){
            ...
            for (int i = 0; i < num_particles; i++) {
                ...
                // heat up section is 18 cm, so if particle position is less than -21, set  Q to 0, T does not change! 
                if (DEMSim.GetOwnerPosition(i).y < -21) {
                    Q_values[i] = 0;
                }
