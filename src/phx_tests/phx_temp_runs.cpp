...
int main(int argc, char* argv[]) {
    ...
    if (argc != 2) {
        std::cout << "Usage: DEM_phx_temp_runs <run_type>" << std::endl;
        return 1;
    }

    // get pin type from input parameters
    int run_num = std::stoi(argv[1]);

    std::string input_particle_positions = "../../input_particle_positions/teardrop_run" + std::to_string(run_num) + ".csv";
    ...
    auto mat_type_sand = DEMSim.LoadMaterial({{"E", 1e8}, {"nu", 0.3}, {"CoR", coeff_res}, {"mu", fric_coeff}, {"Crr", 0.01}});
    auto mat_type_plate = DEMSim.LoadMaterial({{"E", 2e8}, {"nu", 0.3}, {"CoR", coeff_res}, {"mu", fric_coeff}, {"Crr", 0.01}});
    ...
    // my_force_model->SetPerOwnerWildcards({"Temp", "Q"});
    ...
    std::vector<float3> pin_centers = ReadPinPositions("sim_data/pin_pos_run" + std::to_string(run_num) + ".csv");
    std::cout << "number of pins: " << pin_centers.size() << std::endl;


    for (auto pin_center : pin_centers) {

        auto pin = DEMSim.AddWavefrontMeshObject("../data/mesh/tear_drop_run" + std::to_string(run_num) + ".obj", mat_type_plate);
        ...
        pin->AddGeometryWildcard("Temp", std::vector<float>(num_tri, init_temp_cyl));
    ...
    // particles->AddOwnerWildcard("Q", std::vector<float>(num_particles, 0));
    // particles->AddOwnerWildcard("Temp", std::vector<float>(num_particles, init_temp_sand));
    ...
    // Disable contacts within plate components
    DEMSim.DisableContactBetweenFamilies(plate_family, plate_family);

    // prescribe motion
    DEMSim.SetFamilyPrescribedPosition(recylcled_family, "none", "Y+24", "none");
    ...
    DEMSim.Initialize();
    // DEMSim.SetFamilyOwnerWildcardValue(recylcled_family, "Temp", init_temp_sand);
    // DEMSim.SetFamilyOwnerWildcardValue(recylcled_family, "Q", 0.f);

    path out_dir = current_path();

    out_dir += "/phx_periodic_temp_teardrop_Qpfp_smallerE_run" + std::to_string(run_num);
    ...
    for (double t = 0; t < (double)time_end; t += step_size, curr_step++) {
        ...
        if (curr_step % out_steps == 0){
            ...
            for (int i = 0; i < num_particles; i++) {
                ...
                if (DEMSim.GetOwnerPosition(i).y > -4.f ) {
