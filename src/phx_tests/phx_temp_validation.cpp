...
int main(int argc, char* argv[]) {
    ...
    // overwrite the values for constant backplate temp scenario 
    ...
    oss << std::scientific << std::setprecision(1) << "Test_" << TestID << "_larger_ks";
    out_dir += oss.str();
...
if (overlapDepth > 0) {
    ...
    if (curr_step % 2000 == 0 && AOwnerFamily != 2 && BOwnerFamily != 2 && AGeo != 0 && BGeo != 0 && AGeo != 1 && BGeo != 1 && AGeo != 2 && BGeo != 2) {
        ...
        if (AGeo == 3 && (int)myContactType == 11) {
            ...
            T_j = Temp_B[BGeo];
            Q_ij = 40. * ks * radius_contact * (T_j - T_i);
