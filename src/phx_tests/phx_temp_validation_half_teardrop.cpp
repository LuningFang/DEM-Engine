...
int main(int argc, char* argv[]) {
    ...
    if (argc != 4){
        std::cout << "Usage: ./phx_temp_validation_half_teardrop <Boundary condition 0 - constant flux, 1 - constant temp> <TestID 1-4> <pin scale, 0.5 - 1.8>" << std::endl;
        return 1;
    }
    ...
    if (std::abs(pin_scale - 1) < 1e-5) {
        ...
        input_particle_positions = "Oct_Test_1_pin_scale_" + std::string(argv[3]) + "/settling/settled.csv";

    }


    // Append the formatted parameters to out_dir
    std::string out_dir = "Dec_validation/half_teardrop/pin_scale_" + std::string(argv[3]) + "/Test_" + std::to_string(TestID) + "/";
    ...
    // Add pins
    std::vector<float3> pin_centers = ReadPinPositions("sim_data/pin_pos_run2.csv");
    ...
    for (auto pin_center : pin_centers) {
        auto pin = DEMSim.AddWavefrontMeshObject("../data/mesh/pins/teardrop_run1.obj", mat_type_wall);
    ...
    // initialize temeprature of the sand based of the position of particles, it should follow the slope of the backplate temperature
...
if (overlapDepth > 0) {
    ...
    if (curr_step % T_update_frequency == 0 && AOwnerFamily != 2 && BOwnerFamily != 2 && AGeo != 0 && BGeo != 0 && AGeo != 1 && BGeo != 1 && AGeo != 2 && BGeo != 2) {
        ...
        // printf("AGeo: %d, BGeo: %d, myContactType: %d\n", AGeo, BGeo, (int)myContactType);
        ...
        else if ((int)myContactType == 2 && AOwnerFamily == 0){
            // sand is bodyA, pin is body B, look up temp of B based on position of A
            T_j = backplate_temp_slope * AOwnerPos.y + backplate_temp_intercept;
            T_i = Temp_A[AGeo];
            Q_ij = 4. * ks * radius_contact * (T_j - T_i);

        } else if ((int)myContactType == 2 && BOwnerFamily == 0) {
